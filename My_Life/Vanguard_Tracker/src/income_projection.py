...
Description:
    ...
    a specified stock ticker using real-time market data. It takes the user's 
    available settlement fund balance, verifies if a 2 or 3-share limit order 
    trap can be placed, and outputs the exact execution parameters required 
    for Vanguard.
"""
...
def calculate_stock_trap(ticker_symbol, available_cash):
    """
    Pulls YTD data for the chosen ticker, finds the lowest trading floor, 
    and determines how many shares can be safely bought with the current cash reserve.
    """
    ...
    if historical_df.empty:
        print(f"[ERROR] Could not retrieve historical data for {ticker_symbol}. Check if the ticker symbol is correct.")
        return
...
if __name__ == "__main__":
    # --- CONFIGURATION ZONE ---
    # You can change the stock ticker and your cash balance right here!
    target_ticker = "MMM"
    my_settlement_balance = 285.64
    # --------------------------
    
    # Run the analyzer with your chosen settings
    calculate_stock_trap(target_ticker, my_settlement_balance)